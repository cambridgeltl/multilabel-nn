...
def _argparser(positional, optional):
    ...
    for arg, val in _variables(optional):
        ...
        if type_ in (int, float, str):
            ...
            add('--'+name, metavar=_typename(val), type=type(_item(val)),
...
def _item(l, default_type=int):
    assert isinstance(l, list), 'internal error'
    if l:
        return l[0]
    else:
        return default_type()
...
def _typename(d):
    d = _item(d) if isinstance(d, list) else d
